...
async def score_application(
    ...
    try:
        ...
        response = ScoringResponse(
            ...
            pd_score=decision_result['pd_score'],  # Use capped PD from decision engine
            ...
            adverse_action_reasons=adverse_reasons if adverse_reasons else None,
            model_version=state.model_version,
            processing_time_ms=processing_time
        )
        
        logger.info(f"Scored application {response.application_id}: "
                   f"PD={decision_result['pd_score']:.4f}, Decision={decision_result['decision']}")
